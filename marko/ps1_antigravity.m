...
for t = 1:time
    for j = 1:numInd
        ...
        if t < time
             ...
             z_i(j, t+1) = exp(par.rho * log(z_i(j,t)) + sqrt(cpar.sigmae) * randn());
        end
...
R_min = 1.0001; % Must be > 1 to avoid complex numbers in capital demand (MPK = R-1 > 0)
R_max = 1/par.beta;
...
time_GE = 300;
z_i = zeros(numInd, time_GE);
z_i(:,1) = 1;
% draw shocks of productivities across individuals now (outside the loop)
for t = 1:time_GE-1
    for j = 1:numInd
        z_i(j, t+1) = exp(par.rho * log(z_i(j, t)) + sqrt(cpar.sigmae) * randn());
    end
...
while abs(diff) > tol && iter < max_iter 
    ...
    % Assuming delta = 0, so MPK = r = R - 1
    
    k_d = (par.alpha / (R_guess - 1))^(1/(1-par.alpha));
    w_guess = (1 - par.alpha) * (par.alpha / (R_guess - 1))^(par.alpha / (1 - par.alpha));
    par.R = R_guess;
    par.w = w_guess;
    ...
    for t = 1:time_GE
        for j = 1:numInd
            for k = 1:cpar.Nkap
               k_i_int(j,k) = interp_kp(gri.z, kpol_k(k, :), z_i(j,t));
            end
            if t < time_GE
                k_i(j, t+1) = interp_kp(gri.k, k_i_int(j, :), k_i(j,t));
                k_i(j, t+1) = max(0, k_i(j, t+1)); % Enforce borrowing constraint just in case
            end
        end
    end
    k_sim = k_i(:, end);
