...
%   f_opt     - function value at optimal point
    ...
    % Pre-compute golden ratio constant (approx 0.618)
    % This specific ratio ensures that in the next step, one of the interior points
    % can be reused, reducing the number of expensive objective function calls.
    gr = (sqrt(5) - 1) / 2; 
    
    % Initialize interior points c and d
    ...
    while abs(c_high - c_low) > tol && iter < max_iter
        ...
        if fc > fd
            % Maximum is in [a_low, d] -> the interval shrinks from the right
            c_high = d;
            d = c;
            fd = fc;  % Reuse function evaluation - NO new func call needed!
            c = c_high - gr * (c_high - c_low);
            fc = func(c);  % Only ONE new function evaluation per iteration (halves runtime)
        else
            % Maximum is in [c, a_high] -> the interval shrinks from the left
            c_low = c;
            c = d;
            fc = fd;  % Reuse function evaluation - NO new func call needed!
            d = c_low + gr * (c_high - c_low);
            fd = func(d);  % Only ONE new function evaluation per iteration (halves runtime)
        end
