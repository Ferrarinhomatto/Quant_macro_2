...
% VFI_GS: Solves the household problem using Value Function Iteration with Golden Section Search.
%
% Outputs:
...
while iter < cpar.maxit && diff > cpar.tol
    ...
    for j = 1:cpar.N
       ...
       for i = 1:cpar.Nkap
        ...
        % Bounds for C: Must be positive, and cannot exceed total cash-on-hand
        c_min = 1e-6;
        c_max = par.R * k_current + z_current * par.w - 1e-6; % We need investment to be positive

        % Use Golden Section Search to find the consumption level that maximizes the Bellman RHS
        ...
        cpol(j,i) = c_opt;
        k_opt_raw = k_motion(k_current, z_current, cpol(j,i), par);
        
        % === EXTRAPOLATION TOGGLE ===
        % If extrapolation is turned false (for Robustness checks), we enforce
        % that k_opt_raw can never physically exceed the maximum grid point.
        if isfield(cpar, 'extrapolate') && cpar.extrapolate == false
            kpol(j,i) = min(k_opt_raw, max(gri.k));
        else
            kpol(j,i) = k_opt_raw; % Allows saving beyond grid for Setup 1
        end
