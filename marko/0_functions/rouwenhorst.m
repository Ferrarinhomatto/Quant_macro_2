...
% ROUWENHORST: Approximates an AR(1) process with a discrete Markov chain.
...
% Construct the transition matrix iteratively
% The matrix is built by taking a weighted average of shifted N-1 matrices,
% generating a precise binomial-like distribution for the transitions.
for n = 3:N
...
% Compute the discrete states uniformly spaced around the mean
fi = sqrt(N-1)*sigmaz;
Z  = linspace(-fi,fi,N)';
Z  = Z + mu;
if(N==1)
